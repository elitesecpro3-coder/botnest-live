...
- A **build-pipeline gap was found and fixed** the same day: `BotNestWebsite/widget.js` (the file actually served at `https://bot-nest.com/widget.js`) was a second, independently-maintained copy of the widget with no automated link to `apps/widget/src/widget.ts` — meaning the theme/branding work above, even once deployed to `botnest-api`, would **not** have reached the live widget. Traced and fixed in §19; `npm run build:widget` is now the one command that keeps all copies in sync, and the regenerated file is committed on this branch (not deployed).
- Current total: **153 tests, 153 pass** in `apps/api` (up from 119 after the original hardening work — 18 for the theme/branding sanitizer, 12 for lead dedupe, 4 confirming the two new migrations are safe to apply in any order/combination — see §16 and §19).
...
2. Rebuild the widget: from the repo root, `npm run build:widget` (see §19 — this now syncs `apps/widget/dist/widget.js`, `apps/api/dist/widget.js`, and `BotNestWebsite/widget.js` in one command; already run and committed on this branch). Commit and push, then deploy `botnest-website` (it has no build step of its own — see §19 — so a plain push to whatever branch its Git integration watches, or `vercel deploy --prod` from `BotNestWebsite`, ships exactly what's committed). Until this is deployed, setting `widget_theme`/`show_powered_by` on any bot has no visible effect — the config API would return the new fields, but the live widget script at `bot-nest.com/widget.js` doesn't know to read them yet.
3. Deploy the updated `botnest-api` code (dedupe is API-only and needs no widget rebuild; theme/branding need both this and step 2 — and they deploy as two independent Vercel projects, so both steps are required, not either/or).
...
## 19. Widget build/deploy pipeline — traced 2026-09-25 (this is how it actually works, not how it might look)

**Source of truth:** `apps/widget/src/widget.ts` (TypeScript). `npm run build:widget` at the repo root (`tsc` inside `apps/widget`, then one `node -e` copying the single output to both consumption points) is now the one command that keeps every copy in sync — added this session because it didn't exist before.

**The three copies and what actually serves what:**
| File | What it is | Actually used live? |
|---|---|---|
| `apps/widget/dist/widget.js` | The real build output (git-committed) | No — nothing serves this file directly |
| `apps/api/dist/widget.js` | A copy, read by `apps/api/src/server.ts`'s `/widget.js` route | No — `server.ts` isn't the deployed entrypoint (`apps/api/api/index.ts` is, per `vercel.json`), and that entrypoint 302-redirects `/widget.js` to `bot-nest.com/widget.js` rather than serving a file |
| `BotNestWebsite/widget.js` | A separate, independently git-committed copy | **Yes — this is the file served at `https://bot-nest.com/widget.js`**, because `botnest-website`'s Vercel project (`vercel project inspect`, confirmed live) has Root Directory `BotNestWebsite`, an empty Build Command, and Output Directory `.` — it is a pure static deploy with no build step at all |

**Before this fix**, `BotNestWebsite/widget.js` had exactly 2 commits in its entire history; one (`4d8ffc7`) happened to update it in the same commit as `apps/widget/dist/widget.js`, by hand — nothing enforced that, and the commit immediately before this one (`c47e9c5`, the theme/branding/dedupe work) did NOT update it, deliberately, pending this trace.

**A corrected assumption from earlier in this document:** `botnest-api`'s Vercel project shows "Build Command: `npm run vercel-build` or `npm run build`" in `vercel project inspect` — that looked like it meant the repo-root `npm run build` script runs automatically on every `botnest-api` deploy. **It does not.** `apps/api/vercel.json` defines an explicit legacy `builds` array (`{"src": "api/index.ts", "use": "@vercel/node"}`), and a `vercel.json` with an explicit `builds` array takes over the entire build process for that project — Vercel does not additionally run the dashboard-level "Build Command" in that case; `@vercel/node` transpiles `api/index.ts` (and whatever it `require()`s at runtime) directly. Confirmed two ways: (1) the root `npm run build` script currently **fails** outright on pre-existing `apps/api` `tsc` type errors (untyped Supabase client, unrelated to this work — same errors documented as "tolerated" via `transpile-only` for the test suite throughout this document), yet (2) the live deployment demonstrably runs current `apps/api/src` code (every route change in this document went live without that script ever succeeding). Both facts are only consistent if Vercel isn't running that script for `botnest-api` at all. **Practical effect: even `botnest-api`'s own deploy does not, and never did, automatically produce or move `widget.js` anywhere.** The widget has always been, and remains, something a human builds and commits before a deploy — this fix makes that a single reliable command instead of a manual multi-file copy.

**What this means for deploy order:** merging/deploying `botnest-api` alone will **never** make theme/branding appear on `bot-nest.com/widget.js` — that requires a separate deploy of `botnest-website` with the rebuilt file committed, per §18 step 2.
